...
class SliceDiagnostic(OpenPMDDiagnostic):
    """
    Class that defines the field diagnostics to be performed.
    """

    def __init__(self, period=None, fldobject=None, z0=None, comm=None,
                 fieldtypes=["E", "B"], write_dir=None,
    ...
    def write_hdf5( self, iteration ):
        ...
        position = (self.z0 - zmin)/dz
        self.idx_l = int(position)
        self.idx_r = self.idx_l + 1
        # self.r = ((zmin+self.idx_r*dz)-self.z0)/dz
        self.r = self.idx_r - position

        if self.idx_l < 0 or self.idx_r >= Nz:
        ...
        self.create_file_empty_meshes(
            fullpath, iteration, time, Nr, 1, self.z0, dz, dt )
    ...
    def write_dataset( self, field_grp, path, quantity ) :
        ...
        if self.rank == 0:
            ...
            mode0 = mode0.T
            dset[0, :, 0] = (self.r*mode0[:, self.idx_l].real + (1-self.r)*mode0[:, self.idx_r].real)
        ...
        for m in range(1,self.fld.Nm):
            ...
            if self.rank == 0:
                ...
                mode = mode.T
                dset[2*m-1,:,0] = (self.r*2*mode[:,self.idx_l].real + (1-self.r)*2*mode[:,self.idx_r].real)
                dset[2*m,:,0] = (self.r*2*mode[:,self.idx_l].imag + (1-self.r)*2*mode[:,self.idx_r].imag)
